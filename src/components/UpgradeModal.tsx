...
export default function UpgradeModal({ isOpen, onClose, targetPlan = "constellation", descriptionOverride }: UpgradeModalProps) {
  ...
  const modalClassName = isSingularity
    ? "fixed left-1/2 top-1/2 z-[201] w-full max-w-md -translate-x-1/2 -translate-y-1/2 rounded-2xl border border-amber-500/30 dark:border-amber-400/20 bg-white/90 dark:bg-neutral-900/90 backdrop-blur-2xl shadow-[0_0_60px_rgba(99,102,241,0.22)] dark:shadow-[0_0_80px_rgba(168,85,247,0.3),0_0_40px_rgba(251,191,36,0.15)] p-6"
    : "fixed left-1/2 top-1/2 z-[201] w-full max-w-md -translate-x-1/2 -translate-y-1/2 rounded-2xl border border-white/20 dark:border-purple-500/20 bg-white/90 dark:bg-neutral-900/90 backdrop-blur-2xl shadow-[0_0_60px_rgba(99,102,241,0.15)] dark:shadow-[0_0_60px_rgba(168,85,247,0.2)] p-6";

  const iconWrapperClassName = isSingularity
    ? "w-10 h-10 rounded-xl bg-gradient-to-br from-indigo-500/25 dark:from-purple-500/25 to-amber-500/20 border border-amber-500/30 flex items-center justify-center shadow-[0_0_24px_rgba(99,102,241,0.22),0_0_16px_rgba(251,191,36,0.15)] dark:shadow-[0_0_24px_rgba(168,85,247,0.3),0_0_16px_rgba(251,191,36,0.15)]"
    : "w-10 h-10 rounded-xl bg-indigo-500/20 dark:bg-purple-500/20 border border-indigo-500/30 dark:border-purple-500/30 flex items-center justify-center shadow-[0_0_20px_rgba(99,102,241,0.18)] dark:shadow-[0_0_20px_rgba(168,85,247,0.2)]";

  const ctaClassName = isSingularity
    ? "hover:cursor-pointer flex items-center justify-center gap-2 w-full py-3.5 rounded-xl font-medium text-white bg-gradient-to-r from-indigo-600 dark:from-purple-600 to-amber-600 hover:from-indigo-500 dark:hover:from-purple-500 hover:to-amber-500 shadow-[0_0_28px_rgba(99,102,241,0.35),0_0_16px_rgba(251,191,36,0.2)] dark:shadow-[0_0_28px_rgba(168,85,247,0.45),0_0_16px_rgba(251,191,36,0.2)] hover:shadow-[0_0_36px_rgba(99,102,241,0.45),0_0_20px_rgba(251,191,36,0.25)] dark:hover:shadow-[0_0_36px_rgba(168,85,247,0.5),0_0_20px_rgba(251,191,36,0.25)] transition-all"
    : "hover:cursor-pointer flex items-center justify-center gap-2 w-full py-3.5 rounded-xl font-medium text-white bg-indigo-600 dark:bg-purple-500 hover:bg-indigo-700 dark:hover:bg-purple-600 shadow-[0_0_24px_rgba(99,102,241,0.35)] dark:shadow-[0_0_24px_rgba(168,85,247,0.4)] hover:shadow-[0_0_32px_rgba(99,102,241,0.45)] dark:hover:shadow-[0_0_32px_rgba(168,85,247,0.5)] transition-all";
          ...
          >
            <div className="flex items-center justify-between mb-5">
              <div className="flex items-center gap-2">
                <div className={`${iconWrapperClassName}`}>
                  <Sparkles className={isSingularity ? "text-amber-400 dark:text-amber-300" : "text-indigo-500 dark:text-purple-400"} size={22} />
                </div>
            ...
            <ul className="space-y-2 mb-6">
              {features.map(({ icon: Icon, text }) => (
                ...
                >
                  <Icon size={16} className={isSingularity ? "text-amber-500 dark:text-amber-400 shrink-0" : "text-indigo-500 dark:text-purple-400 shrink-0"} />
                  {text}
                </li>
              ))}
            </ul>
